...
export default function Youtube({roomDetails}) {
  console.log("Project Details from Taskboard Tab" + roomDetails[0]);

  const roomIdPut = roomDetails[0]?._id;

  const [youtubeLinks, setYoutubeLinks] = useState(roomDetails[0]?.youtubeLinks || []);
  ...
  const notes = youtubeLinks; // Create a new array with the existing notes

  notes.push({ id: Date.now(), title: Title, description: Description });

  setYoutubeLinks(notes);
  console.log("Notes: " + youtubeLinks);
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    const updatedNotes = youtubeLinks.filter((note) => note.id !== noteId);
  
    setYoutubeLinks(updatedNotes);
  ...
  return (
    <>
      <div className="bg-white p-4 min-h-screen rounded-xl">
        ...
        <div className="grid xl:grid-cols-6 md:grid-cols-4 grid-cols-2 gap-4 mt-5">
          
          {youtubeLinks?.map((taskboard) => {
            //MMMM d, yyyy h:mm a
            const date = format(taskboard.id, "MMMM d, yyyy")
            return (
              <>
              <div  key={taskboard.id}  className='flex  items-center justify-center'>  <div className="rounded border border-gray-300 p-3 shadow-md w-full bg-white h-full text-black">
              <div className="flex w-full items-center justify-between ">
                <a href={taskboard.description} target="_blank" rel="noopener" className="text-gray-800 flex hover:text-gray-600 hover:underline items-center space-x-3 ">
